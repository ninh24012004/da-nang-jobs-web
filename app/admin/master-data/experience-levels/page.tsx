...
export default function ExperienceLevelsPage() {
    ...
    const fetchExperienceLevels = useCallback(async () => {
        try {
            setIsLoading(true);
            const response = await experienceLevelService.getAllExperienceLevels();
            if (response.success) {
    ...
    const openModal = (level: ExperienceLevelResponse | null = null) => {
        if (level) {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (editingLevel) {
                response = await experienceLevelService.updateExperienceLevel(editingLevel.id, formData);
            } else {
                response = await experienceLevelService.createExperienceLevel(formData);
            }
            ...
            toast.error(error.response?.data?.message || "Lỗi khi lưu cấp độ kinh nghiệm");
        } finally {
    ...
    const confirmDelete = async () => {
        ...
        try {
            setIsSubmitting(true);
            const response = await experienceLevelService.deleteExperienceLevel(levelToDelete.id);
            if (response.success) {
                toast.success(response.message);
            ...
            toast.error(error.response?.data?.message || "Lỗi khi xóa cấp độ kinh nghiệm");
        } finally {
            setIsSubmitting(false);
        }
    };
        ...
        <div className="max-w-[1200px] mx-auto space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Danh sách các cấp độ kinh nghiệm để phân loại tin tuyển dụng và hồ sơ.</p>
                </div>
                <button
                    onClick={() => openModal()}
            ...
            <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
                <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="px-4 py-2 bg-white border border-gray-100 rounded-xl text-xs font-medium text-gray-500 shadow-sm">
                            Tổng cộng: <span className="text-[#006B7A] font-bold">{experienceLevels.length}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-x-auto custom-scrollbar">
                    {isLoading ? (
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="text-[11px] uppercase tracking-wider text-gray-400 font-bold border-b border-gray-50">
                                    ...
                                    <th className="px-8 py-5 min-w-[240px]">Cấp độ</th>
                                    <th className="px-8 py-5 text-right w-32">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredLevels.map((level, index) => (
                                    <tr key={level.id} className="group hover:bg-gray-100 transition-colors duration-200">
                                        ...
                                        <td className="px-8 py-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-bold text-gray-900">{level.levelName}</span>
                                            </div>
                                        </td>
                        ...
                        <div className="flex flex-col items-center justify-center py-32 text-center">
                            ...
                            <h3 className="text-lg font-bold text-gray-900">Không tìm thấy cấp độ kinh nghiệm</h3>
                ...
                <div className="p-6 border-t border-gray-50 bg-gray-50/20 text-[11px] text-gray-400 flex items-center gap-2">
                    ...
                    Cấp độ kinh nghiệm giúp lọc chính xác yêu cầu công việc khi tạo tin tuyển dụng và hồ sơ ứng viên.
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden">
                        <div className="p-8 border-b border-gray-100 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-1">
                                    {editingLevel ? "Cập nhật thông tin cấp độ kinh nghiệm." : "Tạo cấp độ kinh nghiệm mới cho hệ thống."}
                                </p>
                            </div>
                            <button
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700">Tên cấp độ</label>
                                <input
                                    value={formData.levelName}
                                    onChange={(e) => setFormData({ levelName: e.target.value })}
                                    placeholder="Nhập tên cấp độ kinh nghiệm"
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                                ...
                                >
                                    {isSubmitting ? "Đang lưu..." : editingLevel ? "Lưu thay đổi" : "Tạo cấp độ"}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden">
                        <div className="p-8 space-y-6 text-center">
                            ...
                            <h3 className="text-2xl font-bold text-gray-900">Xóa cấp độ này?</h3>
                            <p className="text-gray-500">Hành động này sẽ xóa cấp độ kinh nghiệm <span className="font-semibold">{levelToDelete?.levelName}</span>. Bạn có chắc chắn?</p>
                            <div className="flex flex-col sm:flex-row gap-3 justify-center">
                                ...
                                >
                                    {isSubmitting ? "Đang xóa..." : "Xóa cấp độ"}
                                </button>
                            </div>
                        </div>
                    </div>
