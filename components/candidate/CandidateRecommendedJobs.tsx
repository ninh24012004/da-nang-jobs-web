...
export default function CandidateRecommendedJobs() {
  ...
  return (
    <section className="bg-gradient-to-b from-teal-50/20 via-white to-white py-16 md:py-24 border-t border-gray-50">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div className="max-w-3xl text-left space-y-4">
            ...
            <p className="text-gray-500 text-sm md:text-base font-light">
              DN JOBS Core tự động phân tích Ngành nghề quan tâm &amp; Kỹ năng chuyên môn của bạn để gợi ý những công việc tốt nhất tại Đà Nẵng.
            </p>
          </div>
          <Link
            href="/candidate/profile?tab=preferences"
            className="flex items-center gap-1.5 self-start md:self-end px-4 py-2.5 bg-white hover:bg-gray-50 text-[#006b7a] hover:text-[#005a66] border border-gray-200 rounded-xl text-xs font-bold transition-all shadow-xs active:scale-[0.98]"
          >
            <Sliders size={13} />
        ...
        {isLoadingProfile ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div
                key={idx}
                className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm flex flex-col justify-between h-[300px] animate-pulse"
              >
                <div className="space-y-4">
                  {/* Top row: Logo shape and dates/badges */}
                  <div className="flex items-center justify-between gap-4">
                    <div className="h-14 w-14 bg-teal-50/50 rounded-xl" />
                    <div className="space-y-1.5 flex flex-col items-end">
                      <div className="h-3 w-16 bg-gray-150 rounded" />
                      <div className="h-4.5 w-20 bg-emerald-50/50 rounded" />
                    </div>
                  </div>

                  {/* Title & Company name */}
                  <div className="space-y-2">
                    <div className="h-5 w-5/6 bg-gray-150 rounded-md" />
                    <div className="h-3.5 w-1/2 bg-gray-100 rounded-md" />
                  </div>

                  {/* Salary and Location */}
                  <div className="space-y-2.5 pt-2 border-t border-gray-50">
                    <div className="h-4 w-1/3 bg-teal-50/60 rounded-md" />
                    <div className="h-3.5 w-2/3 bg-gray-100 rounded-md" />
                  </div>

                  {/* Tag pills */}
                  <div className="flex gap-1.5 pt-2">
                    <div className="h-5 w-12 bg-teal-50/55 rounded-md" />
                    <div className="h-5 w-16 bg-teal-50/55 rounded-md" />
                    <div className="h-5 w-14 bg-amber-500/5 rounded-md" />
                  </div>
                </div>

                {/* Apply Link button */}
                <div className="h-10 w-full bg-gray-50 rounded-xl mt-4" />
              </div>
            ))}
          </div>
        ) : !hasPreferences ? (
          <div className="bg-white rounded-3xl border border-dashed border-gray-300 p-8 md:p-12 text-center max-w-2xl mx-auto shadow-sm space-y-6">
            <div className="h-16 w-16 bg-[#006b7a]/10 text-[#006b7a] rounded-2xl flex items-center justify-center mx-auto">
              <Sparkles className="h-8 w-8 text-amber-500 fill-amber-500/25" />
            </div>
            <div className="space-y-2">
              <h3 className="text-base font-extrabold text-gray-800">Chưa thiết lập gợi ý việc làm</h3>
            ...
            <Link
              href="/candidate/profile?tab=preferences"
              className="inline-flex items-center justify-center gap-1.5 px-6 py-3 bg-[#006b7a] hover:bg-[#005a66] text-white text-xs font-bold rounded-xl shadow-md transition-all active:scale-[0.98] cursor-pointer"
            >
              <Sliders size={14} />
          ...
          <div className="bg-white rounded-3xl border border-dashed border-gray-300 p-8 md:p-12 text-center max-w-2xl mx-auto shadow-sm space-y-6">
            <div className="h-16 w-16 bg-amber-500/10 text-amber-600 rounded-2xl flex items-center justify-center mx-auto">
              <Sparkles className="h-8 w-8 text-amber-500 fill-amber-500/25" />
            </div>
            <div className="space-y-2">
              <h3 className="text-base font-extrabold text-gray-800">Chưa tìm thấy việc làm phù hợp</h3>
              <p className="text-xs text-gray-500 max-w-md mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recommendedJobs.map((job) => (
              <div
                key={job.id}
                className="bg-white rounded-2xl border border-[#006b7a]/25 shadow-md shadow-teal-50/30 p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between space-y-6 text-left group"
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="text-right">
                      <span className="flex items-center gap-1 text-[10px] text-gray-400 font-light">
                        <Calendar size={10} />
                        {formatTime(job.createdAt)}
                      </span>
                      <span className="inline-flex items-center gap-0.5 mt-1 px-2.5 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[9px] font-extrabold uppercase tracking-wider border border-emerald-200">
                  ...
                  <div className="space-y-1">
                    <Link
                      href={`/jobs/${job.id}`}
                      className="block font-extrabold text-gray-800 text-sm md:text-base leading-snug tracking-tight hover:text-[#006b7a] transition-colors line-clamp-1 cursor-pointer"
                    >
                      {job.jobTitle}
                    </Link>
                  ...
                  <div className="space-y-2 pt-1.5 text-xs text-gray-600 border-t border-gray-50">
                    <div className="flex items-center gap-2">
                      <DollarSign size={14} className="text-[#006b7a] flex-shrink-0" />
                      <span className="font-bold text-[#006b7a] text-sm">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {/* Render Category/Skill names */}
                    {(job.skillNames || []).slice(0, 2).map((skill, tIdx) => (
                      <span
                        key={`s-${tIdx}`}
                        className="px-2.5 py-1 rounded-md bg-[#006b7a]/5 text-[#006b7a] text-[9px] font-bold border border-[#006b7a]/10"
                      >
                        {skill}
                      </span>
                    ))}
                    {(job.categoryNames || []).slice(0, 1).map((cat, tIdx) => (
                      <span
                        key={`c-${tIdx}`}
                        className="px-2.5 py-1 rounded-md bg-amber-500/5 text-amber-600 text-[9px] font-bold border border-amber-500/10"
                      >
                        {cat}
                      </span>
                    ))}
                ...
                <div className="flex items-center gap-2">
                  <Link
                    href={`/jobs/${job.id}`}
                    className="flex-grow py-2.5 bg-gray-50 border border-gray-150 hover:bg-[#006b7a] hover:border-[#006b7a] text-gray-650 hover:text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 transition-all active:scale-[0.98]"
                  >
                    <span>Xem chi tiết</span>
                    <ArrowRight size={12} className="group-hover:translate-x-0.5 transition-transform animate-none" />
                  </Link>
                </div>
              </div>
            ))}
